...
import { FiSearch, FiStar, FiClock, FiCheckCircle, FiXCircle } from 'react-icons/fi';
...
const Reviews = () => {
    ...
    // Fetch reviews
    const { data: reviews, isLoading, isFetching } = useGetReviewsQuery({ query: queries });
    ...
    const handleAction = (type: 'approve' | 'reject' | 'delete' | 'view', review: any) => {
        if (type === 'view') {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-2xl font-bold text-blue-900">{reviews?.meta?.total || 0}</p>
                            </div>
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-2xl font-bold text-yellow-900">
                                    {reviews?.data?.filter((review: any) => review.status === 'pending').length || 0}
                                </p>
                            </div>
                ...
                <Card className="bg-gradient-to-br from-green-50 to-green-100 border-green-200 shadow-lg hover:shadow-xl transition-all duration-300">
                    <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-green-600 text-sm font-medium">Approved Reviews</p>
                                <p className="text-2xl font-bold text-green-900">
                                    {reviews?.data?.filter((review: any) => review.status === 'approved').length || 0}
                                </p>
                            </div>
                ...
                <Card className="bg-gradient-to-br from-red-50 to-red-100 border-red-200 shadow-lg hover:shadow-xl transition-all duration-300">
                    <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-red-600 text-sm font-medium">Rejected Reviews</p>
                                <p className="text-2xl font-bold text-red-900">
                                    {reviews?.data?.filter((review: any) => review.status === 'rejected').length || 0}
                                </p>
                            </div>
                ...
                <CardContent className="p-6">
                    <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-end">
                        ...
                        <div className="flex-1 space-y-2">
                            <label className="text-sm font-medium text-gray-700">Search Reviews</label>
                        ...
                        <div className="space-y-2">
                            ...
                            <Select value={activeTab} onValueChange={(value) => setActiveTab(value as string | 'all')}>
            ...
            <ReviewDetailsModal isOpen={isViewModalOpen} onClose={() => setIsViewModalOpen(false)} review={currentReview} />
